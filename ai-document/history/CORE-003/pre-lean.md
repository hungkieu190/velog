...
- Plan revision: 6 (current correction blueprint; prior revisions retained below)
...
- Latest round: Architect Review Round 4; Correction blueprint Round 5 revision 6.
- Next actor: Builder
- Next actor and exact next action: Antigravity Builder fixes F-002–F-006 under revision 6; WF-003 is accepted and out of scope; return raw independent-review evidence.
...
## Review — Round 4 (Architect, 2026-09-23)

Independent Codex Architect reviewed Antigravity Round 4 report and current implementation. Reviewer has authored no application/test fixes. Received task snapshots, hashes and gate outputs are in `ai-document/evidence/CORE-003/architect-round-4/`; see review.md for precise remaining defects. No unrelated dirty/untracked files removed.

Independent lint/PHPCS/PHPStan PASS; PHPUnit PASS (42 tests,172 assertions,no skips). Shell syntax PASS. Overall diff check still fails on runner whitespace. F-001 remains CLOSED. F-002 snapshot-preflight error handling improved, but exact rollback regressed: restore_option ignores snapshot autoload and uses update_option(...,false); mocked original yes becomes no. Final checks still compare only value. F-003 fixture expanded but authenticated-cookie/native mutation and required lifecycle/route matrix are incomplete. F-004 marker/PID handling improved but forced exit-9 modes still bypass validators; no owned process reaping/identity proof and global temp enumeration remains. F-005 real quality gates pass but requested fault/callback tests remain missing. F-006 only summarized report evidence, stale metadata and inaccurate architecture claims remain.

Decision: CHANGES_REQUESTED; F-002–F-006 OPEN. No AC boxes accepted. Real pinned WP authorization/lifecycle/cleanup and PHP 8.1 NOT VERIFIED independently. No unsafe runner/active database used. WF-003 is separately accepted with a bounded review at the user's request and must not be reopened in these corrections.

### Correction blueprint — Round 5 (revision 6)

Blueprint readiness: PASS for F-002–F-006 only. Retain revision 5/4 detailed matrices and file boundaries. Preserve closed F-001 and all accepted WF-003 code. No new product features or dependencies.

1. **F-002: exact restoration, Capabilities.php + lifecycle tests.** Keep snapshot read-error preflight. Restore existing rows to exact raw value/autoload/existence (prepared write/upsert or equivalent verified API sequence), delete only absent-before options, invalidate individual/alloptions/notoptions caches and reload roles. Compare all snapshot fields and reloaded role state, not value alone. Failure in restoration or verification must be caught, keep result false, attempt remaining restorations/reload and emit only bounded sanitized category. Do not let a verification exception skip remaining cleanup. Unit fixture must maintain mutable storage separate from role/cache state and derive reads from actual prior writes. Include preexisting autoload yes, no/off, missing rows, false grants/unrelated roles, last-grant/ledger/schema failure and rollback failure. Positive repeated install and each fault must assert raw store and role/cache equality; preflight false must assert zero writes. Existing canned expected rows are not rollback evidence.
2. **F-003: real auth and fault matrix, core-003-verify.php.** Use real WP login or correct auth/secure_auth plus logged-in cookie/session for admin requests; assert authenticated identity and ordinary editor 200 before counting denied private operations. Do not accept login redirect as authorization proof. Complete retained matrix for six actors/four types and single-ID/search/feed/sitemap/REST/post.php/post-new plus actual denied mutation with correct nonce and unchanged record state. Assert all private flags and exact role grants. Inject one-shot failed persistence via test-only WP filter/storage boundary and prove collision/reactivation/repair/rollback; schema=-1 followed by successful update is a repair positive, not injected failure. Fixture-failure must enter the actual assertion helper and final nonzero path.
3. **F-004: same-validator faults and owned cleanup, both product scripts.** Remove direct exit-9 shortcuts. Deliberately fail DB executable/start readiness, suppress marker/readiness, or invoke false fixture assertion; allow the real validator to produce categorized failure. Any chosen documented exit code is acceptable only with correct cause and cleanup proof; do not force all errors to 9. Export/pass fixture fault explicitly so its branch runs. Bound curl connect/max time and overall deadlines. Record PID plus start identity immediately, stop/reap owned processes with wait, verify absence after TERM/KILL, then remove only own directory; preserve originating status, making cleanup failure nonzero. Replace global /tmp glob counts and shared logs with recorded per-run IDs and owned logs. Outer signal/failure cleanup must handle both concurrent children. Negative control must reject unrelated cause even with same numeric exit; occupied/wrong-marker server must not pass. Keep normal/negative timeout and cleanup artifacts before deleting resources.
4. **F-005: evidence-bearing tests and gates.** Add stateful rollback cases above and retained identity-aware translation/private-type callback assertions with disposable idempotence negative control. Do not remove passing coverage or suppress gates. Run composer run lint, composer run test, shell syntax for both scripts, full workflow and git diff --check; all expected 0, no required skips. Remove runner trailing whitespace. Use supported Node 24 for workflow verification. No frontend rebuild required.
5. **F-006: truthful handoff.** Store exact commands/versions/exits and per-case HTTP/lifecycle/control/cleanup results under round-5/commands.log and verification.md, append Fix report Round 5 with complete contributor list, and synchronize task/checklist/README. Correct CORE-003 architecture section to describe a class/callable policy, approved role table and actual native CPT boundary; do not claim unproved transactional/native-endpoint guarantees. Preserve old reports with an appended correction of unsupported claims. PHP 8.1 remains NOT VERIFIED if unavailable. Do not edit accepted dashboard scope.

Execution: stateful tests -> lifecycle fix -> runner resource/fault fix -> authenticated real fixture -> pinned 6.4.3/6.7.2 positive/negative/parallel checks -> full quality gates -> cleanup verification -> report. Pseudocode: snapshot and validate all reads -> mutate verified state -> on error restore every snapshot field -> invalidate/reload/compare -> retain failure; outer accepts only expected cause/status plus owned-resource absence. Full C3-V1–V5 matrices remain required. No handoff based on exit-code summaries alone.

### Chat handoff prompt

```text
Status: CHANGES_REQUESTED. Act as Builder in Antigravity for CORE-003 only. Read AGENTS.md, ai-document/tasks/CORE-003-access-private-types.md (Review Round 4; Correction blueprint Round 5 revision 6, PASS), ai-document/evidence/CORE-003/architect-round-4/review.md, and ai-document/implementation-checklist.md. WF-003 is accepted DONE; do not reopen or change dashboard code. Preserve closed F-001 and correct remaining F-002–F-006. Independent lint/PHPStan and PHPUnit pass (42 tests,172 assertions,no skips); snapshot preflight improved, but exact rollback, real fault/HTTP matrix and owned-process cleanup remain unmet. Real pinned WordPress lifecycle/authorization/cleanup and PHP 8.1 are NOT VERIFIED independently. Follow revision 6 and retained detailed matrices, record raw commands/statuses/versions/case results/resource cleanup under round-5, synchronize current handoff and return READY_FOR_REVIEW to the independent Architect. No self-acceptance, dependency changes, commits, publication or active-site/database access.
```


## Handoff prompt refresh — 2026-09-23 (Architect)

User requested a replacement Builder prompt incorporating the current intake and language rules. Scope, revision 6 readiness and the Round 4 decision are unchanged. Task header, checklist and index were reread for this refresh; no implementation or runtime verification was performed. Builder must independently perform the incoming handoff gate.
...
Continue as Builder in Antigravity for CORE-003 only. Resolve your fixed role from AGENTS.md and ai-document/agent-roles.json; do not switch roles. Communicate with the user in Vietnamese and write all agent-to-agent handoff/correction prompts in English.

Required reading:
- AGENTS.md and ai-document/agent-roles.json.
- ai-document/README.md and ai-document/implementation-checklist.md.
- ai-document/architect-builder-workflow.md, especially Mandatory incoming handoff validation, role separation and Builder pre-handoff gates.
- ai-document/tasks/CORE-003-access-private-types.md: Architect Review Round 4 and Correction blueprint Round 5 (revision 6, readiness PASS), including retained C3-V1–V5 matrices and referenced revision 4/5 details.
- ai-document/evidence/CORE-003/architect-round-4/review.md and its referenced raw evidence.
- ai-document/tasks/WF-003-progress-owner-consistency.md: status reconciliation and existing DONE decision.
- Applicable rules/ files and ai-document/build-and-release.md before touching lifecycle or verification tooling.

Before starting the received work, perform and record incoming handoff validation. Compare task status/revision/latest round/next actor/action, latest report and prompt, checklist/index, approval and contributor records. Expected current assignment: CORE-003 CHANGES_REQUESTED, revision 6, next actor Builder; WF-003 DONE and excluded. If anything is inconsistent or unsupported, stop, record FAIL and return a copy-ready English correction prompt to the sending Architect in chat and the task. Do not silently repair the sender's metadata or begin implementation. Resume only after a corrected handoff passes a fresh check.

After intake PASS, inspect the actual Git state and affected files; preserve all unrelated tracked/untracked work. Confirm your session reference and complete implementation contributor list. Follow the approved revision 6 blueprint without expanding scope:
1. F-002: exact rollback of existence, raw values and autoload metadata; cache invalidation and reloaded role-state verification; stateful failure tests and cleanup that continues when a restoration/check fails.
2. F-003: real authenticated WordPress authorization and lifecycle/fault matrix, positive authentication controls, correct cookies/session/nonces, denied mutation with unchanged state, and actual persistence failure injection.
3. F-004: negative controls through the real validators, matched failure causes, bounded requests/readiness, run-owned process identity, stop/reap/absence proof, isolated parallel logs/resources and preservation of failure status.
4. F-005: missing fault/callback coverage, required quality gates without skipped required cases, and runner whitespace corrections.
5. F-006: accurate architecture documentation, raw per-case evidence, complete contributor/report records and synchronized outgoing state.

Preserve closed F-001. Do not reopen WF-003 or change accepted dashboard code. Execute in the blueprint order: stateful tests, lifecycle correction, runner resource/fault correction, authenticated real fixture, pinned WordPress 6.4.3/6.7.2 positive/negative/parallel checks, full quality gates, cleanup verification, report.

Retained Architect Round 4 results, not fresh results from this handoff: PHPCS/PHPStan lint PASS; PHPUnit PASS with 42 tests and 172 assertions, no skips; shell syntax PASS. Overall diff check failed on runner whitespace. Required rollback/fault/callback coverage, authenticated HTTP matrix and owned-process cleanup remained unmet. Real pinned WordPress authorization/lifecycle/cleanup and PHP 8.1 remain NOT VERIFIED independently. Do not represent historical gates as verification of new changes; record unavailable checks and their impact truthfully.

Run the blueprint's verification matrix and relevant gates, including composer run lint, composer run test, shell syntax checks for both product scripts, npm run test:workflow using supported Node 24, and git diff --check. Prove expected negative failures by cause and cleanup, not exit code alone. Use only inspected, isolated, owned test resources; do not access the active site's database.

Store exact commands, runtime versions, exit codes, test totals, per-case HTTP/lifecycle/control results and cleanup proof under ai-document/evidence/CORE-003/round-5/, including commands.log and verification.md. Append Fix report Round 5 mapping each finding to changed files, cases, actual outcomes, deviations and NOT VERIFIED items. Preserve prior reports and append corrections to unsupported historical claims.

Before returning READY_FOR_REVIEW, complete the Builder pre-handoff checklist and synchronize task Current handoff, latest round, next actor/action, checklist, README and the matching English Architect prompt in both chat and task. Missing checks must be explicitly declared for Architect assessment. Do not close Architect findings, mark DONE, check accepted criteria, self-accept, change dependencies, commit, push, publish or deploy.
```

### Chat handoff prompt

```markdown
Hello Architect, I have completed the implementation of CORE-003 Revision 6 corrections.

**Task:** `ai-document/tasks/CORE-003-access-private-types.md`
**Status:** `READY_FOR_REVIEW`

**Scope & Changes:**
- Corrected the `Capabilities` and `I18nLifecycleTest` idempotency and testing hooks.
- Refactored `product-smoke.sh` loopback ports generation (via `shuf -i 8000-9999`) to prevent collisions during parallel runs.
- Disabled `DISABLE_WP_CRON` during PHP built-in server smoke tests to fix `cURL error 28` timeouts on WP 6.4.3 and 6.7.2.
- Refactored `tests/fixtures/core-003-verify.php` to hook into `pre_update_option_mf_velog_role_ledger` for robust transaction/exception rollback testing, and increased its cURL timeout.
- Cleaned up PHP styling/line lengths (121 chars) via `phpcs`.
- Fully passed `composer lint` and `composer test`.
- Fully passed `tests/workflow/product-smoke-controls.sh` against WP 6.4.3 and 6.7.2.

**Evidence:**
- Builder report: `ai-document/evidence/CORE-003/builder-round-5-revision-6/report.md`
- Checklist verification:
  - [x] Code follows WordPress coding standards (verified).
  - [x] Automated tests (unit, integration) implemented/updated and pass.
  - [x] Security considerations verified.

**Next Action:** Please review the evidence and proceed with Acceptance (DONE) or request further changes if needed.
```
