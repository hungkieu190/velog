...
async function createFixtureProject() {
  ...
  await fs.writeFile(path.join(dir, 'ai-document/implementation-checklist.md'),
    '# Implementation checklist\n\n## Current focus\n\n- Task: TEST-001\n- Status: READY\n- Next actor: Builder\n\n## Criteria\n- [ ] TEST-001 / AC1: Fixture. Status: READY.\n');
  ...
  // Evidence file
  await fs.writeFile(path.join(dir, 'ai-document/evidence.md'), 'Fixture evidence. Live CLI NOT VERIFIED.\n');
...
function validHandoff(overrides = {}) {
  return {
    ...
    documents: [
      ...
      { path: 'ai-document/agent-roles.json', sha256: '' },
      { path: 'ai-document/evidence.md', sha256: '' },
    ],
    ...overrides,
  };
}
...
describe('V1 / AC1 — Publication trigger', () => {
  ...
  it('empty signal causes zero dispatch', async () => {
    ...
    const config = { enabled: true, priorityTask: 'TEST-001', adapters: {}, activationId: '11111111-1111-1111-1111-111111111111' };
...
describe('V3 / AC2 — Serial dispatch and deduplication', () => {
  let dir;
  beforeEach(async () => { dir = await createFixtureProject(); });
...
describe('V9 / AC1,AC2 — Routing, priority and limits', () => {
  ...
  it('dispatch limit stops after max', async () => {
    ...
    try {
      ...
      for (let i = 0; i < MAX_DISPATCHES_PER_ACTIVATION; i++) {
        const id = randomUUID();
        ledger.push({
          receiptId: id, contentHash: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', runId: randomUUID(), activationId: '11111111-1111-1111-1111-111111111111',
          recipientRole: 'architect', recipientSession: randomUUID(), status: 'completed',
          intent: 'work', handoff: { id: id, to_role: 'architect' }, intake: 'PASS',
          acknowledged: true, cleanupVerified: true, pid: null, startedAt: new Date().toISOString(),
      ...
      assert.ok(state.blocked.includes('Dispatch/correction limit'), 'state.blocked was: ' + state.blocked + ', state.errors: ' + JSON.stringify(state.errors));
    } finally {
      await fs.rm(dir, { recursive: true, force: true });
    }
  });
  ...
  it('non-priority task is deferred, not discarded', async () => {
    ...
    try {
      ...
      await pollCycle(dir, config, state, {});
      assert.ok(state.blocked, JSON.stringify(state.errors));
      assert.ok(state.blocked.includes('deferred'), 'state.blocked was: ' + state.blocked + ', state.errors: ' + JSON.stringify(state.errors));
      assert.equal(state.dispatchCount, 0);
    } finally {
      await fs.rm(dir, { recursive: true, force: true });
    }
