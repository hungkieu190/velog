...
process.stdout.write(${JSON.stringify(events.join('\n') + '\n')});`);
  let persisted = false;
  let spawned = false; let inputExistedBeforePersistence = false;
  const response = await (variant.antigravity ? antigravity : codex)({ projectRoot: root, config: { executable, sessionId }, handoff, prompt: 'EXACT_APPROVED_PROMPT_SENTINEL', runId: randomUUID(), deadline: 2000,
    onSpawn: async () => { spawned = true; inputExistedBeforePersistence = await fs.access(path.join(root, 'input.txt')).then(() => true, () => false); await delay(150); persisted = true; } });
  const row = { name, inputExistedBeforePersistence, success: response.success, cleanupVerified: response.cleanupVerified ?? null, error: response.error ?? null, spawned, persistenceAwaited: persisted, exactPromptDelivered: (await fs.readFile(path.join(root, 'input.txt'), 'utf8').catch(() => '')).includes('EXACT_APPROVED_PROMPT_SENTINEL') };
  ...
  await delay(160);
  if (!variant.antigravity) await fs.cp(path.join(root, '.cache/handoff/runs'), path.join(out, 'adapter-logs', name), { recursive: true });
