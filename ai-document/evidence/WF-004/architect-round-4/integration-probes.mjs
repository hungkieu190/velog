...
try {
  ...
  await pollCycle(root,config(),state,{builder:{dispatch:async()=>({success:false,cleanupVerified:true,agentIntake:'FAIL',agentOutcome:'blocked',result:{receipt_id:first.id,task_id:'TEST-001',intake:'FAIL',outcome:'blocked',new_receipt_id:null}})}});
  ...
  let correction;
  try {await publishHandoff(options(root,{intent:'correct_metadata',fromRole:'builder',toRole:'architect',rejectedReceiptId:first.id}));let launches=0; await pollCycle(root,config(),state,{architect:{dispatch:async()=>{launches++;return {success:true,cleanupVerified:true};}}}); correction={published:true,launches,blocker:state.blocked};}
  ...
  results.push({name:'actual-dashboard-entrypoint',get:get.status,head:head.status,post:post.status,monitor,blockerVisibleInHtml:html.includes('its CLI interface is not verified'),ledgerExists,exit:code,signal});
